...
const nums: Array<number> = []; // 숫자롤 된 배열타입 반환
const colors: Array<string> = ["234"]; // 문자열로 된 배열타입 반환
...
// 위와같이 매번 타입을 바꿔서 비슷한 함수를 만드는 것은 비효율적이다.

// function identity(item:any):any{
// return item;
// }
// 그렇다고 위와 같이 any타입의 함수를 만들면, input output 타입이 일치하지 않는다.
// 바로 이 경우에 제네릭 함수가 사용된다.
...
identity<number>(7); //function identity<number>(item: number): number
...
console.log(getRandomElement<string>(["a", "b", "c"])); //함수를 돌릴 때 마다 랜덤으로 배열 내의 문자열 반환
getRandomElement<number>([23, 5235, 6, 3, 42, 646, 235]); //function getRandomElement<number>(list: number[]): number
...
// merge에 들어갈 값들 자체의 타입이 문자열이든 숫자든 배열이든지 상관없지만 꼭 객체여야 한다고 선언한것.
// console.log(merge({name:'Colt'},9)) => error
...
const strings = makeEmptyArray<string>(); // function makeEmptyArray<string>(): string[]
...
function makeEmptyArray3<T = number>(): T[] {
  return [];
}
...
// "name" in pet  =>name 이 pet안에 있나요? 라고 묻는 식으로 타입 좁히기가 가능하다.
...
//세가지 같은 요소들을 가진 인터페이스가 있을 때 어떻게 구분할까?
...
/* ---------------------------------- 소진검사 ---------------------------------- */
//위의 코드와 함께 보자.
//*never타입은 어디든 할당되지만 never에는 어떤 타입도 할당할 수 없는 에러를 활용하자.
...
type FarmAnimal2 = Pig | Rooster | Cow |Sheep;
  ...
  switch (animal.kind) {
    ...
    case "rooster":
      ...
      default:
        ...
        const _exhaustiveCheck:never=animal; // 이 코드가 에러가 나게 되면서 개발자가 알게된다.
